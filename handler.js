...
export async function groupsUpdate(groupsUpdate) {
  ...
  for (const groupUpdate of groupsUpdate) {
    ...
    if (groupUpdate.subject) text = (chats.sSubject || this.sSubject || conn.sSubject || '```Subject has been changed to```\nالموضوع').replace('@subject', groupUpdate.subject);
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        await mconn.conn.sendMessage(nk.from, {contacts: {displayName: 'العم سكونا', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '*[ 🍊 ] اضغط هنا علشان*', body: 'تتابعني على الانتسغرام', thumbnail: imagen1, sourceUrl: 'https://www.instagram.com/alkaser7008'}}}}, aa);
